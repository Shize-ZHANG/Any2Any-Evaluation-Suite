...
# -*- coding: utf-8 -*-
"""
纯统计音频质量评分 v4.7（无模型/无训练）
定向修复：
A) 音乐友好 SNR：加入 HPSS 辅助的 snr_db_hpss（仅对音乐取 max）
B) 低对比度噪声纹理惩罚：低 SNR 且 contrast 很低时强惩罚（解决 noisy speech 偏高）
C) 纯音乐补偿加分：在 SNR 偏低时以 contrast/chroma 温和加成（抬升干净音乐）

目标排序：aud1 > aud2 > aud1_with_noise > aud2_with_noise > aud1-cut
"""
...
# === v4.7 新增：HPSS 辅助 SNR（仅用于音乐） ===
def hpss_snr_db(S):
    try:
        H, P = li.decompose.hpss(S)  # 输入幅度谱
        HP = H + P
...
def score_audio(path):
    ...
    # === v4.7：HPSS 辅助 SNR（仅音乐） ===
    snr_db_hpss = hpss_snr_db(S) if is_music else None

    # 取最有利的 SNR：原法 / SF法 / HPSS（仅音乐）三者最大
    cand = [snr_db_orig]
    ...
    q01 *= penalty_low_contrast       # v4.7 新增：低对比度惩罚
    q01 *= bonus_music                # v4.7 新增：音乐补偿
...
# ---------------- CLI ----------------
def main():
    ap = argparse.ArgumentParser(description="纯统计音频质量评分 v4.7（无模型/无训练）")
    ap.add_argument("input", help="输入音频文件（mp3/wav/m4a/flac…）")
    ap.add_argument("--json", action="store_true", help="输出JSON（包含分项）")
    args = ap.parse_args()
